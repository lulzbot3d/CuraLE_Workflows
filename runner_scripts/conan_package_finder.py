...
def search_conan_packages(pattern: str) -> str:
    ...
    Args:
        pattern: Conan package pattern (e.g., "*/*@lulzbot/curale_12824")
...
def validate_and_normalize_jira_ticket(ticket: str) -> str:
    ...
    # If it's just a number, prepend "curale_"
    if re.match(r'^[0-9]+$', normalized_ticket):
        normalized_ticket = f"curale_{normalized_ticket}"
    ...
    if keyword not in ['cura', 'np', 'pp']:
        raise ValueError(f"Invalid Jira ticket keyword '{keyword}'. Expected one of: curale, np, pp")
...
def categorize_packages(packages: List[str]) -> Tuple[str, List[str]]:
    """
    Separate curale packages from override packages.
...
def main():
    ...
    if args.jira_ticket and args.search_pattern:
        ...
        try:
            # Conditionally validate based on --validate-input flag
            if args.validate_input:
                normalized_ticket = validate_and_normalize_jira_ticket(args.jira_ticket)
            else:
                # Skip validation, just normalize basic format
                normalized_ticket = args.jira_ticket.lower()
